...
{
    std::string cmds_list[] = {"KICK",    "INVITE", "TOPIC", "MODE", "JOIN",
                               "PRIVMSG", "NICK",   "USER",  "PART"};
        ...
        for (size_t i = 0; i < pc->args[1].size(); i++)
        {
            if (!std::isprint(static_cast<unsigned char>(pc->args[1][i])))
                throw std::invalid_argument(
                    "Unprintable character in topic text");
        }
    }
}
...
void IsValid_PRIVMSG(ParsedCommand* pc)
{
    ...
    for (size_t i = 0; i < pc->args[1].size(); i++)
    {
        if (!std::isprint(static_cast<unsigned char>(pc->args[1][i])))
            throw std::invalid_argument(
                "Unprintable character in message text");
    }
}
    ...
    if (pc->args.size() == 3)
    {
        for (size_t i = 0; i < pc->args[2].size(); i++)
        {
            if (!std::isprint(static_cast<unsigned char>(pc->args[2][i])))
                throw std::invalid_argument(
                    "Unprintable character in kick comment");
        }
    }
}
...
void IsValid_NICK(ParsedCommand* pc)
{
    ...
    if (pc->args.empty() || pc->args.size() != 1)
        throw std::invalid_argument(
            "Wrong number of parameters to set a nickname");
    // nickname length
...
void IsValid_MODE(ParsedCommand* pc)
{
    ...
    if (sign == '-')
    {
        if (pc->args.size() != 2)
            throw std::invalid_argument(
                "Mode does not take a parameter when disabled");
        return;
    }
